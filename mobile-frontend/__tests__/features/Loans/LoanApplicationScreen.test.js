...
import LoanApplicationScreen from "../../../src/features/Loans/LoanApplicationScreen";

const mockApplyLoan = jest.fn();
jest.mock("../../../src/services/apiService", () => ({
  __esModule: true,
  default: { applyLoan: (...args) => mockApplyLoan(...args) },
}));
...
describe("LoanApplicationScreen", () => {
  beforeEach(() => {
    ...
    mockApplyLoan.mockReset();
    mockApplyLoan.mockResolvedValue({ data: { id: "loan1" } });
  ...
  it("displays loading indicator during submission", async () => {
    ...
    // Hold the API call open so the button stays in its submitting state.
    let resolveApply;
    mockApplyLoan.mockReturnValueOnce(
      new Promise((resolve) => {
    ...
    await waitFor(() => expect(findState()?.disabled).toBe(true));

    resolveApply({ data: { id: "loan1" } });

    await waitFor(() => expect(findState()?.disabled).toBe(false));
  });
});
