...
describe("LoginScreen", () => {
  ...
  it("shows validation errors for invalid input", async () => {
    const { findByText, findAllByText, getByText, getByLabelText } = render(
      <LoginScreen navigation={mockNavigation} />,
      { wrapper: AllTheProviders },
    );
    ...
    // Submitting empty marks fields touched; both required messages appear.
    fireEvent.press(loginButton);
    const requiredMessages = await findAllByText("Required");
    ...
    fireEvent.press(loginButton);
    await findByText("Password too short");
  });
