...
const mockTheme = CombinedLightTheme; // full app theme (borderRadius, fontSizes, spacing, colors)
...
describe("ProfileScreen", () => {
  ...
  it("allows editing name and email and simulates saving changes", async () => {
    ...
    // updateProfile received the edited values
    expect(mockUpdateProfile).toHaveBeenCalledWith({
      name: "Updated Name",
      email: "updated@example.com",
    });
    await waitFor(() => {
      // edit mode has exited, so the Name input is no longer present
      expect(queryByLabelText("Name")).toBeNull();
    });
  });
  ...
  it("calls toggleTheme when dark mode switch is pressed", () => {
    ...
    fireEvent(darkModeSwitch, "valueChange", true); // RN Switch fires valueChange
