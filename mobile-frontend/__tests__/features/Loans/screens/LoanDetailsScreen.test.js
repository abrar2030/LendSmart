...
describe("LoanDetailsScreen", () => {
  beforeEach(() => {
    ...
    getLoanDetails.mockImplementation((id) => {
      const loan = placeholderLoans.find((l) => l.id === id);
      return loan
        ? Promise.resolve({ data: loan })
  ...
  it("renders loading state initially then loan details", async () => {
    ...
    await findByText("Loan Request: $1500");
    await findByText("Purpose: Debt Consolidation");
    await findByText("8.5% APR");
    await findByText("0x123B...sabc");
    await findByText(
      "Looking to consolidate high-interest credit card debt into a single, manageable loan.",
    );
    await findByText("$0 / $1,500");
  });
  ...
  it("shows error message if loan is not found", async () => {
    ...
    await findByText("Loan not found");
  });
  ...
  it('shows "Connect Wallet to Fund" button if wallet not connected and loan is available', async () => {
    ...
    await findByText("Connect Wallet to Fund");
  });
